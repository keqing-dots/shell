...
Item {
    ...
    function setPosition() {
        if (!root.width || !root.height || !contentLoader.item)
            return;
        var contentW = contentLoader.item.implicitWidth || contentLoader.item.width || 300;
        var contentH = contentLoader.item.implicitHeight || contentLoader.item.height || 200;
